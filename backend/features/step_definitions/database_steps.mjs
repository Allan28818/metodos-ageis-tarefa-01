...
Given('a tabela {string} existe', async function (tableName) {
  ...
  const tableExists = tables.some(row => Object.values(row).includes(tableName));
...
Then('um pool de conexões deve ser criado com limite de {int}', function (limit) {
...
Then('as configurações devem incluir host, user, password e database', function () {
...
Then('os campos devem ser na ordem: nome, senha_usuario, email, telefone', function () {
  // Verificação conceitual
});
...
// ESTRUTURA DA TABELA
Then('ela deve ter o campo {string} como INT AUTO_INCREMENT', async function (campo) {
...
Then('deve ter o campo {string} como VARCHAR\\({int}) NOT NULL', function (campo, size) {
  // Verificação conceitual
});
...
Then('deve ter o campo {string} como DECIMAL\\({int},{int}) com default {int}', function (campo, p, s, defaultVal) {
  // Verificação conceitual
});

Then('deve ter o campo {string} como DATETIME com default CURRENT_TIMESTAMP', function (campo) {
  // Verificação conceitual
});
...
Then('não deve haver overhead de criar nova conexão a cada requisição', function () {
  // Conceitual
});
...
Then('o database deve ser {string}', function (database) {
  ...
  this.expect(defaultDb).to.be.oneOf(['dbEmprestivoLivro', 'dbEmprestimoLivro']);
});
...
Then('a senha armazenada deve começar com {string}', function (prefix) {
  this.expect(this.dbUser.senha_usuario).to.match(new RegExp(`^${prefix.replace(/\$/g, '\\$')}`));
});
...
Then('campos sensíveis como telefone não devem ser retornados desnecessariamente', function () {
...
Then('os campos inseridos devem ser: nome, senha_usuario, email, telefone', function () {
  // Conceitual
});
