...
Given('que eu tenho dados de usuário completos exceto o campo {string}', function (campo) {
  this.requestData = {
    nome: 'Teste Usuário',
...
Then('o usuário deve estar salvo no banco de dados com matrícula {string}', async function (matricula) {
  const user = await database.getUserByEmail(this.testEmail);
  this.expect(user).to.not.be.undefined;
...
Given('que eu registro um usuário com os seguintes dados:', async function (dataTable) {
  const data = dataTable.rowsHash();
  this.requestData = {
    nome: data.nome,
    email: data.email,
...
Then('a senha armazenada deve começar com {string}', function (prefix) {
  this.expect(this.dbUser.senha_usuario).to.match(new RegExp(`^${prefix.replace('$', '\\$')}`));
});
