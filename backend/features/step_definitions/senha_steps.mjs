...
Given('o sistema está configurado com {int} salt rounds', function (saltRounds) {
  this.saltRounds = saltRounds;
  this.expect(this.saltRounds).to.equal(10);
});

// REGISTRO E CRIPTOGRAFIA
Given('que um novo usuário se registra com a senha {string}', async function (password) {
...
Given('que um usuário se registra com a senha {string}', async function (password) {
  this.plainPassword = password;
  this.testEmail = 'teste' + Date.now() + '@email.com';
...
Given('outro usuário se registra com a mesma senha {string}', async function (password) {
...
Then('a senha armazenada não deve ser igual a {string}', async function (plainPassword) {
  const user = await database.getUserByEmail(this.testEmail);
  this.expect(user.senha_usuario).to.not.equal(plainPassword);
});

Then('o hash deve conter o indicador de {int} salt rounds', function (saltRounds) {
  this.expect(this.hashedPassword).to.match(new RegExp(`^\\$2b\\$${saltRounds}\\$`));
});
...
Then('ambos devem começar com {string}', function (prefix) {
  this.expect(this.hash1).to.match(new RegExp(`^${prefix.replace(/\$/g, '\\$')}`));
  this.expect(this.hash2).to.match(new RegExp(`^${prefix.replace(/\$/g, '\\$')}`));
});

// LOGIN E COMPARAÇÃO
Given('que existe um usuário com senha criptografada no banco', async function () {
...
When('o usuário tenta fazer login com senha vazia {string}', async function (password) {
  this.requestData = {
    email: this.testEmail,
    senha: password
  };
...
Given('que diferentes usuários se registram com senhas:', async function (dataTable) {
  this.senhasTestadas = [];
...
Then('todas devem começar com {string}', function (prefix) {
  this.hashesGerados.forEach(item => {
    this.expect(item.hash).to.match(new RegExp(`^${prefix.replace(/\$/g, '\\$')}`));
  });
});
...
Then('a única forma de verificar deve ser usando bcrypt.compare', async function () {
  const isMatch = await bcrypt.compare(this.plainPassword, this.hashedPassword);
...
Then('os primeiros {int} caracteres devem ser {string}', function (count, prefix) {
...
Then('os últimos {int} caracteres devem ser o hash', function (count) {
  ...
  const hashPart = this.hashedPassword.substring(this.hashedPassword.length - count);
