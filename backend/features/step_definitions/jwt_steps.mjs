...
When(
  "o sistema gera um token JWT para este usuário no momento T1",
  function () {
    this.token1 = authService.generateToken(this.userId);
...
When(
  "o sistema gera outro token JWT para o mesmo usuário no momento T2",
  async function () {
...
Then(
  "o tempo de expiração deve ser equivalente a {int} horas",
  function (hours) {
    const decoded = jwt.decode(this.token);
...
Then(
  "o payload deve conter o campo {string} com valor {string}",
...
// LOGIN E TOKEN
Given(
  "que existe um usuário cadastrado com email {string} e senha {string}",
...
// DADOS SENSÍVEIS
Given(
  "que existe um usuário com id {string} e senha {string}",
